...
def train_sam(
    ...
    for epoch in range(1, cfg.num_epochs):
        ...
        for iter, data in enumerate(train_dataloader):
            if epoch > 1 and epoch % cfg.eval_interval == 0 and not validated or epoch==2:
            ...
            pred_masks, iou_predictions = model(images, masks)
            num_masks = sum(len(pred_mask) for pred_mask in pred_masks)
            loss_focal = torch.tensor(0., device=fabric.device)
            loss_dice = torch.tensor(0., device=fabric.device)
            loss_iou = torch.tensor(0., device=fabric.device)
            for pred_mask, gt_mask, iou_prediction in zip(pred_masks, labels, iou_predictions):
                batch_iou = calc_iou(pred_mask, gt_mask)
                loss_focal += focal_loss(inputs=pred_mask, targets=gt_mask)
                loss_dice += dice_loss(inputs=pred_mask, targets=gt_mask)
                loss_iou += F.mse_loss(iou_prediction, batch_iou, reduction='sum') / num_masks

            loss_total = 20. * loss_focal + loss_dice + 30. * loss_iou
            #loss_total = 50. * loss_focal
            optimizer.zero_grad()
...
if __name__ == "__main__":
    cfg.num_epochs = 50
    cfg.sam.checkpoint="/root/xyf/sam_vit_h_4b8939.pth"
